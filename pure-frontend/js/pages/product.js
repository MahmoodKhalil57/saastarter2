...
if (!p) detail.innerHTML = '<p>Not found. <a href="./products.html">Back to catalog</a></p>';
  ...
  detail.innerHTML = `
    ...
    <div class="col-md-7 vstack gap-3">
      ...
      <div class="d-flex gap-2 align-items-center">
        <button id="add" class="btn btn-primary btn-lg">Add to cart</button>
        <a id="view-cart" class="btn btn-outline-secondary btn-lg d-none" href="./cart.html">View cart →</a>
        <button id="wish" class="btn btn-link fs-3 p-0 ${wished ? "" : "opacity-50"}" style="filter:${wished ? "none" : "grayscale(1)"}">❤️</button>
      </div>
    </div>`;
  ...
  document.getElementById("wish").onclick = async (event) => {
    const { wished: now } = await toggleWishlist(slug);
    event.target.style.filter = now ? "none" : "grayscale(1)";
    event.target.classList.toggle("opacity-50", !now);
  };
}
...
const paintStars = () => { stars.innerHTML = [1,2,3,4,5].map((n) => `<span role="button" data-n="${n}" class="${n <= rating ? "text-warning" : "text-body-tertiary"}">★</span>`).join(""); };
...
async function renderReviews() {
  ...
  document.getElementById("reviews").innerHTML = rows.length === 0
    ...
    : rows.map((r) => `<div class="card"><div class="card-body py-2"><div class="d-flex justify-content-between"><strong>${r.title ?? "Review"}</strong><span class="text-warning">${"★".repeat(r.rating)}</span></div><p class="mb-1 small text-body-secondary">${r.body ?? ""}</p><small class="text-body-tertiary">— ${r.author_name ?? "Anonymous"}</small></div></div>`).join("");
}
